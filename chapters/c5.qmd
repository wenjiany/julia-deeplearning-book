...
function load_images(folder::String)
    ## images = Vector{Array{RGB{N0f8}, 2}}()  # actually use more memory?
    images = []
...
### too many copies of the data, out-of-memory
...
# layer_to_reset = model.layers[2]
# layer_to_reset[3].weight .= ( Flux.glorot_uniform(size(layer_to_reset[3].weight)...) |> gpu)
...
function reset_params!(params::Flux.Zygote.Params, method::Function=Flux.glorot_uniform)
    n_layers = length(trainable_params)
    for i in 1:n_layers
        params[i] .= (Flux.glorot_uniform(size(params[i])...) |> gpu)
    end, 
end |> gpu
...
## todo: add methods for reseting parameters
lr_finder = function(params, dl::MLUtils.DataLoader, opt, lr_range)
...
### make sure to reset the params, since it's been set to values from lr finding
trainable_params[1] .= (Flux.glorot_uniform(size(trainable_params[1])...) |> gpu)
trainable_params[2] .= (Flux.glorot_uniform(size(trainable_params[2])...) |> gpu)
...
orig_params = deepcopy(Flux.params(model));

lr_range = 10 .^ (collect(range(-7, stop=3, step=0.2) ))

## doesn't work right, need to reset to pretrained params instead of randomly assigning all params

best_lr, lr_losses, lr_plot = lr_finder(Flux.params(model), pets_dl, Adam(0.001), lr_range)
lr_plot
best_lr
...
## accuracy(valid_X[:,:,:, 1:1000], valid_Y[:, 1:1000])
