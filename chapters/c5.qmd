...
check memory usage, we have three copies of the data: 
images, pets::DataBlock, (train_db + valid_db)::DataBlock.

Can we reduce memory usage by using views?
...
loss(x, y) = Flux.crossentropy(model(x), y) |> gpu
trainable_params = Flux.params(model[2])  # Only parameters of the last Dense layer
freezed_params = Flux.params(model[1])  # Only parameters of the last Dense layer
