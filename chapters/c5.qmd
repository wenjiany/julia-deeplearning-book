...
function prepare_flux_data(db::DataBlock, tfms::Augmentor.ImageOperation=nothing)

    if tfms != nothing
        X = batch_tfm.(db.inputBlocks, tfms)
    end
    X = [channelview(dl) for dl in db.inputBlocks] 
...
train_X, train_Y = prepare_flux_data(train_db, p1 |> p2)

valid_X, valid_Y = prepare_flux_data(train_db, p1 |> p2)
