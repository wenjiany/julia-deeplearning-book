...
pets_dl = DataLoader(collect(zip(X, Y)), batchsize=10, shuffle=true)
dataset = first(pets_dl)
...
##     Flux.train!(loss, Flux.params(model), dataset, opt, cb = throttle(evalcb, 10))
    ...
    for (i, dataset) in enumerate(pets_dl)
        data = stack([d[1] for d in dataset], dims=4)
        y = dropdims(stack([d[2] for d in dataset], dims=2), dims=3)
